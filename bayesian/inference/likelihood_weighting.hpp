...
* @brief Infering (Reasoning) any probability $P(Q|E)$ by using Likelihood Weighting.
* @author godai_0519
* @date 06/21/2018
*/

#ifndef BAYESIAN_NETWORKS_INFERENCE_LIKELIHOOD_WEIGHTING_HPP
#define BAYESIAN_NETWORKS_INFERENCE_LIKELIHOOD_WEIGHTING_HPP
...
//! Provide a class performing Likelihood Weighting for bn::network.
...
public:
    ...
    using sample_type = std::unordered_map<component::random_variable_ptr, matrix<double>>;
    ...
    **/
    // TODO: tag
    likelihood_weighting(network<RepresentMethod> const& network, cpt_manager const& cpts)
    ...
    likelihood_weighting(NetworkType&& network, CptManagerType&& cpts, QueryType&& queries = QueryType(), EvidenceType&& evidences = EvidenceType())
    ...
    likelihood_weighting& set_query(query_type queries)
    {
    ...
    likelihood_weighting& set_query(QueryType&& queries)
    {
        if(!is_contained(queries, network_)) throw std::runtime_error(""); // TODO:
        queries_ = std::forward<QueryType>(queries);
        is_modified_ = true;
    ...
    likelihood_weighting& set_evidence(evidence_type evidences)
    {
    ...
    sample_type run(std::size_t const iterator_num)
    {
        ...
        for(std::size_t i = 0; i < iterator_num; ++i)
        {
            ...
            double weight;
            std::tie(sample, weight) = make_weighted_sample(network_, cpts_, evidences_, topological_sorted_);

            for(auto const& q : queries_)
                working_accepted_samples[q][std::vector<std::size_t>{sample.at(q)}] += weight;

            // for(std::size_t j = 0; j < quick_accesser.size(); ++j)
            //     quick_accesser[j][sample[queries_[j]]] += weight;
        }
...
private:
    ...
    void initialize()
    {
        ...
        for(auto const& q : queries_)
        {
            matrix<double> mat(std::vector<std::size_t>{ q->max_value }, 0);
...
#endif // BAYESIAN_NETWORKS_INFERENCE_LIKELIHOOD_WEIGHTING_HPP
