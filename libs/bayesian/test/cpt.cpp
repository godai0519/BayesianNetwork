...
BOOST_AUTO_TEST_CASE(cpt_zero_dimension)
{
    std::random_device rd;
    std::mt19937 mt(rd());
    std::uniform_int_distribution<int> max_value_dist(2, 10);
    ...
    std::vector<double> data(target_rv->max_value);
    for(std::size_t j = 0; j < target_rv->max_value; ++j)
        data[j] = probability_dist(mt);
    std::copy(data.begin(), data.end(), elem1.begin());
    for(std::size_t j = 0; j < target_rv->max_value; ++j)
        BOOST_CHECK_CLOSE(elem1[j], elem2[j], 0.0001);
}

BOOST_DATA_TEST_CASE(cpt_some_dimensions, boost::unit_test::data::xrange(3), parent_num)
{
    std::random_device rd;
    std::mt19937 mt(rd());
    std::uniform_int_distribution<int> max_value_dist(2, 10);
    ...
    auto const target_rv = generate_target(mt, max_value_dist);
    auto const parent_rv = generate_parents(mt, max_value_dist, parent_num + 1);
    ...
    for(std::size_t j = 0; j < parent_rv.size(); ++j)
...
BOOST_DATA_TEST_CASE(cpt_copy, boost::unit_test::data::xrange(4), parent_num)
{
    std::random_device rd;
    std::mt19937 mt(rd());
    ...
    for(std::size_t j = 0; j < cpt.parents().size(); ++j)
